...
public class OrderReportTargetModel implements BaseModel {
    /**
     * 获取填写目标数据
     *
     * @param time
    ...
    public void getData(String time, final GetDataListener listener) {
        ...
        MyHttpClient.postByTimeout(UrlPath.URL_WRITE_DEPOSIT_REPORT_TARGET, null, params, 60, new RequestCallBack<OrderReportTargetBean>() {
            @Override
    ...
    public void saveTarget(String param, String time, final SaveTargetListener listener) {
        ...
        params.put("time", time);
        MyHttpClient.postByCliId(UrlPath.URL_SAVE_DEPOSIT_REPORT_TARGET, null, params, new RequestCallBack<String>() {
            @Override
            public void onFailed(String result) {
                listener.failed(result);
            }
