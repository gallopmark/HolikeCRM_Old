...
//        srl.setRefreshHeader(new WaterDropHeader(mContext));
        ...
        LocalDataSource.clearShopData(mContext);
        LocalDataSource.clearAuthInfo(mContext);
        initData(true);
    }
...
//        tvMsg.setText(HomePagePresenter.getMsgNum(bean.getNewData().getNewsCount()));
            ...
            rvPayMenu.setAdapter(new CommonAdapter<HomepageBean.NewDataBean.CreditItem>(mContext, menuItems) {
                ...
                public void onBindHolder(RecyclerHolder holder, HomepageBean.NewDataBean.CreditItem data, int position) {
                    ...
                    holder.itemView.setOnClickListener(v -> {
                        ...
                        bundle.putSerializable(Constants.TYPE_LIST, homepageBean.getTypeList());
        ...
        /*店铺数据*/
        if (bean.getTypeList() != null && bean.getTypeList().getShopData() != null && !bean.getTypeList().getShopData().isEmpty()) {
            LocalDataSource.saveShopData(mContext, MyJsonParser.fromBeanToJson(bean.getTypeList().getShopData()));
        }
        if (bean.getAuthInfo() != null && !bean.getAuthInfo().isEmpty()) {
            LocalDataSource.saveAuthInfo(mContext, bean.getAuthInfo());
        }
    }
...
//        banner.start();
    ...
    public void noPermissions(final List<HomepageBean.MessageListBean> listBeans) {
        ...
        rvMessage.setAdapter(new CommonAdapter<HomepageBean.MessageListBean>(mContext, listBeans) {
            ...
            @Override
            public void onBindHolder(RecyclerHolder holder, HomepageBean.MessageListBean messageListBean, int position) {
                final TextView tvTpye = holder.obtainView(R.id.tv_item_homepage_message_type);
                TextView tvTitle = holder.obtainView(R.id.tv_item_homepage_message_title);
                TextView tvDate = holder.obtainView(R.id.tv_item_homepage_message_date);
                tvTitle.setText(messageListBean.getTitle());
                tvDate.setText(messageListBean.getTime());
                if (messageListBean.getType() == 2) {
                    tvTpye.setText("公告");
                    tvTpye.setBackgroundResource(R.drawable.bg_tv_item_rv_type1);
                } else {
                    tvTpye.setText("通知");
                    tvTpye.setBackgroundResource(R.drawable.bg_tv_item_rv_type2);
                }
                holder.itemView.setOnClickListener(v -> {
                    if (messageListBean.getType() == 2) {
                        AnnounceFragment.startMessageDetailsActivity(HomepageFragment.this, messageListBean.getMessageId(), REQUEST_CODE);
                    } else {
                        CustomerManageFragment.customerDeatil((BaseActivity) mContext, messageListBean.getPersonalId(), messageListBean.getMessageId());
//                            NotifyFragment.startOrderDetails(HomepageFragment.this, messageListBean.getOrderId(), messageListBean.getOrderId(), REQUEST_CODE);
                    }
                });
            }
        });
                ...
                holder.itemView.setOnClickListener(v -> {
                    ...
                    switch (bean.getName()) {
                        ...
                        case "售后反馈":
                            ...
                            mPresenter.feedback((Activity) mContext, homepageBean);
                            break;
                        case "反馈记录":
