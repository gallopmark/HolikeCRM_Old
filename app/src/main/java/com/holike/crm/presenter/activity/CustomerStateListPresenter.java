...
public class CustomerStateListPresenter extends BasePresenter<CustomerStateListView, CustomerStateListModel> {
    ...
    private List<MultiItem> listBeans = new ArrayList<>();
    ...
    public class CustomerListAdapter extends CommonAdapter<MultiItem> {
        ...
        CustomerListAdapter(Context context, List<MultiItem> mDatas, String stateName) {
        ...
        protected int bindView(int viewType) {
            if (viewType == 1) {
                if (TextUtils.equals(stateName, "意向客户跟进")) {
                    return R.layout.item_rv_customer_state_list_intention;
                } else if (TextUtils.equals(stateName, "待量房客户")) {
                    return R.layout.item_rv_customer_state_list_measure;
                } else if (TextUtils.equals(stateName, "待出图客户")) {
                    return R.layout.item_rv_customer_state_list_out_diagram;
                } else if (TextUtils.equals(stateName, "待签约客户")) {
                    return R.layout.item_rv_customer_state_list_unsigne;
                } else if (TextUtils.equals(stateName, "待收全款客户")) {
                    return R.layout.item_rv_customer_state_list_collect_money;
                } else if (TextUtils.equals(stateName, "待复尺客户")) {
                    return R.layout.item_rv_customer_state_list_remeasure;
                } else if (TextUtils.equals(stateName, "待下单客户")) {
                    return R.layout.item_rv_customer_state_list_order;
                } else if (TextUtils.equals(stateName, "待安装客户")) {
                    return R.layout.item_rv_customer_state_list_uninstall;
                } else if (TextUtils.equals(stateName, "已安装客户")) {
                    return R.layout.item_rv_customer_state_list_installed;
                } else if (TextUtils.equals(stateName, "已流失客户")) {
                    return R.layout.item_rv_customer_state_list_lossed;
                }
        ...
        public void onBindHolder(RecyclerHolder holder, MultiItem multiItem, int position) {
            ...
            if (itemType == 1) {
                ...
                if (TextUtils.equals(stateName, "意向客户跟进")) {
                    ...
                    String statuMove = "客户状态：";
                    if (!TextUtils.isEmpty(dateBean.getStatuMove()))
                        statuMove += dateBean.getStatuMove();
                    tvState.setText(statuMove);
                ...
                holder.itemView.setOnClickListener(v -> CustomerManageFragment.customerDeatil((BaseActivity) mContext, dateBean.getPersonalId(), dateBean.getHouseId()));
            } else {
    ...
    public void setAdapter(RecyclerView recyclerView, String stateName) {
        adapter = new CustomerListAdapter(recyclerView.getContext(), listBeans, stateName);
    ...
    public void onLoadMoreCompleted(List<CustomerStateListBean.DateBean> beans) {
        this.listBeans.addAll(beans);
        adapter.notifyDataSetChanged();
    }

    public void noMoreData() {
        this.listBeans.remove(noMoreBean);
        listBeans.add(noMoreBean);
        adapter.notifyDataSetChanged();
    }
