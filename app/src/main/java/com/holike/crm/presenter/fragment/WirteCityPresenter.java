...
import com.holike.crm.model.fragment.WirteCityModel;
...
public class WirteCityPresenter extends BasePresenter<WirteCityView, WirteCityModel> {

    /**
     * 获取数据
     */
    public void getData() {
        model.getData(new WirteCityModel.GetDataListener() {
            @Override
    ...
    public void saveCity(final WriteCityBean.SelectDataBean selectDataBean, String start, String end, final String time) {
        if (selectDataBean == null) {
            ...
            if (getView() != null)
                getView().loading();
            model.saveCity(selectDataBean.getDealerId(), String.valueOf(ParseUtils.parseLong(start)), String.valueOf(ParseUtils.parseLong(end)), new WirteCityModel.SaveCityListener() {
                @Override
    ...
    public void delCity(String dealerId, final int position) {
        model.delCity(dealerId, new WirteCityModel.DelCityListener() {
            @Override
            public void success(String success) {
                if (getView() != null)
